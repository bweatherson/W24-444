...
:    Does v(o1, Travis) -  v(o2, Travis) > v(o2, Taylor) - v(o1, Taylor) make sense?
...
Assume both sides are positive, so Travis is better off in o1, and Taylor is better off in o2.
...
So it does make sense to ask whether v(o1, Travis) > v(o2, Travis).
...
Ideally, what we'd have is a set of axiomatic constraints on a rule for combining preferences that are collectively satisfied by precisely one combination rule. 

Arrow shows that we can build some plausible rules that are satisfied by precisely zero combination rules. 
